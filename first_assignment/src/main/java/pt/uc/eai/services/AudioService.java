    ...
    public byte[] process(byte[] packet) throws IOException {
        ...
        for (Chunk chunk : payload.getChunks()) {
            for (Frame frame : chunk.getFrames()) {
                double sum = 0;
                for (double sample : frame.getAudioSamples()) sum += sample;
            }
        }
        ...
        if (memUsed <= 0) memUsed = result.length; // Fallback para o tamanho do objeto se o GC interferir

        this.metrics = new ServiceMetrics(
            "AudioService", (endDe - startDe) / 1000000, (endSe - startSe) / 1000000,
